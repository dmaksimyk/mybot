...
module.exports = async (vkId, newDate) => {
  ...
  if ( newDate ) {
    const checkUser = await saveUser.findOne({vk_id: vkId, __v: newDate.__v});
    if ( checkUser ) {
      const value = 3000000000000000;
      if ( newDate.balance >= value) newDate.balance = value;
      newDate.__v += 1;
      await saveUser.updateOne(newDate);
      return 'SUCESSFUL';
    }
    return 'ERR_CODE_1';
  }
  const checkUser = await saveUser.findOne({vk_id: vkId});
  if ( checkUser ) return checkUser;

  // eslint-disable-next-line new-cap
  return await new saveUser(user).save();
};
