...
module.exports = async (msg) => {
  ...
  if (msg.senderId === msg.replyMessage.senderId) return msg.send(`${ment}, вы не можете себя пнуть!`);
  ...
  if (data.tag === user[0].first_name) return msg.send(`${ment}, вы пнули ${(data.mentioning === true) ? `[id${replyUser[0].id}|${replyUser[0].first_name}]` : `${replyUser[0].first_name}`}!`);
  return msg.send(`${ment}, вы пнули ${(data.mentioning === true) ? `[id${data.vk_id}|${data.tag}]` : `${data.tag}` }!`);
};
