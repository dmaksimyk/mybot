...
module.exports = async (msg) => {
  ...
  if (msg.senderId === msg.replyMessage.senderId) {
    return msg.send(`${ment}, вы погладили себя!`);
  };
  ...
  if (data.tag === user[0].first_name) {
    return msg.send(`${ment}, вы погладили ${(data.mentioning === true) ? `[id${replyUser[0].id}|${replyUser[0].first_name}]` : `${replyUser[0].first_name}`}!`);
  };
  return msg.send(`${ment}, вы погладили ${(data.mentioning === true) ? `[id${data.vk_id}|${data.tag}]` : `${data.tag}` }!`);
};
